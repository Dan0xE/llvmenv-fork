...
pub fn official_releases() -> Vec<Entry> {
    // To get all offical LLVM releases, get all tagged releases from LLVM git repo and filter for tags that match X.y.z with x, y and z being numbers.
    //   gh api --method GET /repos/llvm/llvm-project/git/matching-refs/tags | jq '.[].ref' | sed -E 's|"refs/tags/llvmorg-([0-9]+)\.([0-9]+)\.([0-9]+)"|Entry::official(\1, \2, \3),|' | grep 'Entry::official' | sort -Vr
    vec![
